defmodule LetsRaid.PlayersTest do
  use LetsRaid.DataCase

  alias LetsRaid.Players

  import LetsRaid.PlayersFixtures
  alias LetsRaid.Players.{Player, PlayerToken}
  ...
  describe "roles" do
    ...
    test "list_roles/0 returns all roles" do
      role = role_fixture()
      assert Players.list_roles() == [role]
    end

    test "get_role!/1 returns the role with given id" do
      role = role_fixture()
      assert Players.get_role!(role.id) == role
    end

    test "create_role/1 with valid data creates a role" do
      valid_attrs = %{role: "some role", spec: "some spec"}

      assert {:ok, %Role{} = role} = Players.create_role(valid_attrs)
      assert role.role == "some role"
      assert role.spec == "some spec"
    end

    test "create_role/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Players.create_role(@invalid_attrs)
    end

    test "update_role/2 with valid data updates the role" do
      role = role_fixture()
      update_attrs = %{role: "some updated role", spec: "some updated spec"}

      assert {:ok, %Role{} = role} = Players.update_role(role, update_attrs)
      assert role.role == "some updated role"
      assert role.spec == "some updated spec"
    end

    test "update_role/2 with invalid data returns error changeset" do
      role = role_fixture()
      assert {:error, %Ecto.Changeset{}} = Players.update_role(role, @invalid_attrs)
      assert role == Players.get_role!(role.id)
    end

    test "delete_role/1 deletes the role" do
      role = role_fixture()
      assert {:ok, %Role{}} = Players.delete_role(role)
      assert_raise Ecto.NoResultsError, fn -> Players.get_role!(role.id) end
    end

    test "change_role/1 returns a role changeset" do
      role = role_fixture()
      assert %Ecto.Changeset{} = Players.change_role(role)
    end
  end
end
